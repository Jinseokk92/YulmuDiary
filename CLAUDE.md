...
## 보안 이슈 (인증 구현 시 함께 해결 필요)
- **FamilyGroup 접근 제어 미구현**: 소셜 로그인한 모든 사용자가 `babyId=1`의 일기를 작성/조회 가능
  - 원인: 일기 작성·조회 시 요청자가 해당 baby의 `family_group`에 속하는지 검증하지 않음
  - 해결 방향:
    1. 일기 작성 시 `family_membership` 테이블에서 작성자의 family_group 소속 여부 검증
    2. 일기 조회 시 요청자가 해당 family_group 소속인지 검증
    3. 소셜 로그인 후 `invite_code`로 가족 그룹에 참여하는 온보딩 플로우 추가